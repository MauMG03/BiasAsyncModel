...
class BiasedInfluenceGraph(nx.DiGraph):

    def __init__(self, n, ops=[], keep_history=True, G=None, edge_labels='abcdefghijklmnopqrstuvwxyz', influence_fn=lambda self, e: 0.5, bias_fn=lambda self, e: self.opinion[e[0]] - self.opinion[e[1]]):
        if G is None:    
            p = 0.1
    ...
    def execute_edge(self, edge):
        ...
        #self.opinion[callee] = self.opinion[callee] + self.bias_fn(self,(self.opinion[caller] - self.opinion[callee])) * self.influence_fn(self,edge)
        self.opinion[callee] = clamp(self.opinion[callee] + self.bias_fn(self,edge) * self.influence_fn(self,edge))
        if self.keep_history:
